...
<script>
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        return full.unit?full.unit.:null;
                    },
                },
                {
                    targets: 5,
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function(data, type, full, meta) {
                        var delete_url = "{{route('inventory.sell_unit_requests.delete', 'id=:id')}}";
                        delete_url = delete_url.replace(':id', full.id);

                        var view_url = "{{route('inventory.sell_unit_requests.show', ':id')}}";
                        ...
                        @if(auth()->user()->hasPermission('update-inventory-sell-request') || auth()->user()->hasPermission('delete-inventory-sell-request'))
                            ...
                            @haspermission('update-inventory-sell-request')
                                value += `
                                            <a href="{{route('inventory.sell_unit_requests.modals.update')}}" class="dropdown-item" data-toggle="modal" data-target="#fast_modal" data-path="{{route('inventory.sell_unit_requests.modals.update')}}" data-title="{{trans('inventory::inventory.update_sell_request')}}" data-id="` + full.id + `" data-modal-load>
                                                <span>
    ...
    jQuery(document).ready(function() {
        // Init datatable
        sell_requests_table = initTable1("{{ route('inventory.sell_unit_requests.index') }}");
        ...
        $('#m_search').on('click', function(e) {
            ...
            sell_requests_table = initTable1("{{ route('inventory.sell_unit_requests.index') }}" + '?' + query);
        });
