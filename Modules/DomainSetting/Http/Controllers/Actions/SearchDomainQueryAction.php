...
class SearchDomainQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the domains
        $domains = (new Domain)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $domains = $domains->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $domains = $domains->where('created_at', '>=', $from);
                } elseif ($to) {
                    $domains = $domains->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $domains = $domains->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $domains = $domains->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $domains = $domains->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
